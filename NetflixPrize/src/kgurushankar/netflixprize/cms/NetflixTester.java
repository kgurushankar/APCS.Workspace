package kgurushankar.netflixprize.cms;
...
public class NetflixTester {
	...
	public static final String tagsFile = folder + FileIO.fileSeparator + "tags.csv";
	...
	public static void main(String[] args) {
		...
		/*
		 * { int numberTested = 0, userToTest, movieToTest; ArrayList<String>
		 * baseDataLines = FileIO.readFile(baseFile); baseDataLines.remove(0);
		 * 
		 * ArrayList<Integer> users = new ArrayList<Integer>();
		 * ArrayList<ArrayList<Integer>> movies = new ArrayList<ArrayList<Integer>>();
		 * 
		 * for (String line : baseDataLines) { String[] lineVals = line.split(",");
		 * userToTest = Integer.parseInt(lineVals[0]); movieToTest =
		 * Integer.parseInt(lineVals[1]); boolean found = false; for (int i = 0; i <
		 * users.size(); i++) if (users.get(i) == userToTest) {
		 * movies.get(i).add(movieToTest); found = true; break; } if (!found) {
		 * users.add(userToTest); ArrayList<Integer> newList = new ArrayList<Integer>();
		 * newList.add(movieToTest); movies.add(newList); } } numberTested = 0; while
		 * (numberTested < 20) { int testIndex = 0; do { testIndex = (int)
		 * (Math.random() * 2000); } while (users.indexOf(testIndex) >= 0); double val =
		 * tester.getRating(testIndex, 1); if (val >= 0) {
		 * System.out.println("Rating returned from getRating for a non-existing user! "
		 * ); System.out.
		 * println("(This probably means you're reading in the database incorrectly)");
		 * System.exit(0); } numberTested++; } numberTested = 0; while (numberTested <
		 * 20) { int testIndex = (int) (Math.random() * users.size()), testIndex2; do {
		 * testIndex2 = (int) (Math.random() * 200); } while
		 * (movies.get(testIndex).indexOf(testIndex2) >= 0);
		 * 
		 * double val = tester.getRating(users.get(testIndex), testIndex2); if (val >=
		 * 0) { System.out.
		 * println("Rating returned from getRating for a movie that was not rated by user!"
		 * ); System.out.
		 * println("(This probably means you're reading in the database incorrectly)");
		 * System.exit(0); } numberTested++; } numberTested = 0; while (numberTested <
		 * 20) { int testIndex = (int) (Math.random() * users.size()); int testIndex2 =
		 * (int) (Math.random() * movies.get(testIndex).size());
		 * 
		 * int val1 = users.get(testIndex); int val2 =
		 * movies.get(testIndex).get(testIndex2);
		 * 
		 * double val = tester.getRating(val1, val2); if (val < 0) { System.out.
		 * println("No rating returned from getRating for a movie that was rated by user!"
		 * ); System.out.
		 * println("(This probably means you're reading in the database incorrectly)");
		 * System.exit(0); } numberTested++; } }
		 */
